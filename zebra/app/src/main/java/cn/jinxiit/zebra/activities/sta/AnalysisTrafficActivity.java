...
import cn.jinxiit.zebra.fragments.stas.TrafficListFragment;
import cn.jinxiit.zebra.interfaces.MyConstant;
import cn.jinxiit.zebra.toolbars.MyToolBar;

public class AnalysisTrafficActivity extends BaseActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.activity_analysis_traffic);
    ...
    protected void onDestroy()
    {
        super.onDestroy();
        myToolBar = null;
        mFragmentPagerItemAdapter = null;
    }
    ...
    private void choosePlatform(String menu, String platform)
    {
        ...
        {
            int count = mFragmentPagerItemAdapter.getCount();
            for (int i = 0; i < count; i++)
            {
                TrafficListFragment trafficListFragment = (TrafficListFragment) mFragmentPagerItemAdapter.getPage(i);
                if (trafficListFragment != null)
                {
                    trafficListFragment.refreData(platform);
                }
            }
        }
    }
    ...
    {
        mViewPlatforms.setVisibility(View.GONE);
        myToolBar = new MyToolBar(this, "流量分析", "全部平台");
    ...
    private void initSmartTabLayout()
    {
        mFragmentPagerItemAdapter = new FragmentPagerItemAdapter(getSupportFragmentManager(), FragmentPagerItems.with(this)
                .add(R.string.toptitle_yesterday, TrafficListFragment.class, new Bundler().putString(MyConstant.STR_TYPE, MyConstant.STR_YESTERDAY)
                        .get())
                .add(R.string.toptitle_hebdomad, TrafficListFragment.class, new Bundler().putString(MyConstant.STR_TYPE, MyConstant.STR_SEVENDAY)
                        .get())
                .add(R.string.toptitle_month, TrafficListFragment.class, new Bundler().putString(MyConstant.STR_TYPE, MyConstant.STR_MONTH)
                        .get())
                .create());
